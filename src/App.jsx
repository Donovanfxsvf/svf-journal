...
async function fbSaveBranding(data) {
  try { await setDoc(doc(db, "config", "branding"), data, { merge: true }); } catch(e) { console.error("branding save:", e); }
}
...
function Login({logoUrl}) {
  ...
  const logoBlock = (
    <div className="login-logo">
      {logoUrl && <img src={logoUrl} alt="SVF Logo" style={{height:90,objectFit:"contain",borderRadius:16}}/>}
    ...
    <div style={{display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        <label style={{display:"inline-flex",alignItems:"center",gap:6,background:"#161820",border:"1px solid #252830",
          borderRadius:8,padding:"7px 14px",cursor:"pointer",fontSize:12,fontWeight:600,color:"#A0A4B0",transition:"all .15s"}}>
          📁 Seleccionar imagen
        ...
        {saved && <span style={{fontSize:11,color:"#00C076",fontWeight:600}}>✅ Logo actualizado en todos los dispositivos</span>}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(()=>{
    const unsub = onAuthStateChanged(auth, async (firebaseUser) => {
      if(firebaseUser){
        let data = null;
        // Retry up to 3 times to prevent false-null from network issues
        for(let attempt=0; attempt<3; attempt++){
          data = await fbGetUserData(firebaseUser.uid);
          if(data) break;
          if(attempt < 2) await new Promise(r=>setTimeout(r, 800));
        }
        if(!data){
          // BRAND NEW USER — first time ever, create with demo data
          data = {
        ...
        // EXISTING USER — just read, NEVER overwrite anything
        // Check if banned
        if(data && data.banned) {
          await signOut(auth);
  ...
  // Real-time branding listener — logo updates instantly on all devices
  useEffect(()=>{
    const unsub = onSnapshot(doc(db, "config", "branding"), (snap) => {
      if(snap.exists()){
        const d = snap.data();
        if(d.logoBase64) setLogoUrl("data:image/jpeg;base64," + d.logoBase64);
      }
    });
    return ()=>unsub();
  },[]);
  ...
  if(!user) return <><style>{css}</style><Login logoUrl={logoUrl}/></>;
