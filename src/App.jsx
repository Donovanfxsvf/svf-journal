...
async function fbGetStats() {
  try {
    ...
    return snap.exists() ? snap.data() : { userCount: 0, maxUsers: 165 };
  } catch { return { userCount: 0, maxUsers: 165 }; }
}

async function fbSetMaxUsers(n) {
  try {
    await setDoc(doc(db, "config", "stats"), { maxUsers: n }, { merge: true });
  } catch(e) { console.error("setMaxUsers:", e); }
}
...
function Login() {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      // Check user limit — dynamic from Firestore
      const stats = await fbGetStats();
      const limit = stats.maxUsers || MAX_USERS;
      if((stats.userCount||0) >= limit) {
...
function AdminPanel() {
  ...
  useEffect(()=>{
    (async()=>{
      ...
      setStats(s);
      setLimitInput(String(s.maxUsers || MAX_USERS));
  ...
  const handleSaveLimit = async () => {
    ...
    setSavingLimit(true);
    await fbSetMaxUsers(n);
    setStats(prev => ({...prev, maxUsers: n}));
  ...
  const currentMax = stats?.maxUsers || MAX_USERS;
  const pct = ((activeUsers/currentMax)*100).toFixed(1);
    ...
    <div className="content">
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:20}}>
        {[
          ...
          {l:"Cupos Disponibles", v:currentMax-activeUsers,     c:"#64D2FF", icon:"🎟️"},
      ...
      <div style={{background:"#13151D",border:"1px solid #1A1C24",borderRadius:12,padding:"14px 18px",marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
          ...
          <span style={{fontSize:12,fontFamily:"DM Mono",color:"#00C076"}}>{activeUsers} / {currentMax}</span>
        </div>
        <div style={{background:"#1A1C24",borderRadius:20,height:8,overflow:"hidden"}}>
          <div style={{height:"100%",borderRadius:20,width:Math.min(parseFloat(pct),100)+"%",
            background: activeUsers>=currentMax?"#FF3B30":activeUsers>currentMax*0.85?"#FFD60A":"#00C076",
            transition:"width .3s"}}/>
        </div>
      </div>
      ...
      <div style={{background:"#13151D",border:"1px solid #1A1C24",borderRadius:12,padding:"14px 18px",marginBottom:20}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
          ...
          <div>
            <div style={{fontSize:13,fontWeight:700,color:"#E2E4EA"}}>Control de Cupos</div>
            <div style={{fontSize:11,color:"#4A4E5A"}}>Cambia el límite máximo de usuarios sin tocar el código</div>
          </div>
        </div>
        <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,background:"#0A0C10",border:"1px solid #252830",
            borderRadius:10,padding:"8px 14px",flex:1,minWidth:180}}>
            <span style={{fontSize:12,color:"#6A6E7A",fontWeight:600,whiteSpace:"nowrap"}}>Máximo de usuarios:</span>
            <input
              type="number"
              min="1"
              value={limitInput}
              ...
              style={{background:"none",border:"none",outline:"none",color:"#00C076",fontFamily:"DM Mono",
                fontWeight:800,fontSize:18,width:80,textAlign:"right"}}
            />
          </div>
          <button className="btn btn-primary" onClick={handleSaveLimit} disabled={savingLimit}
            style={{padding:"10px 18px",fontWeight:700}}>
            {savingLimit ? "Guardando…" : "💾 Guardar"}
          </button>
          {limitSaved && <span style={{fontSize:12,color:"#00C076",fontWeight:700}}>✅ Guardado</span>}
        </div>
        <div style={{marginTop:10,fontSize:11,color:"#3A3E4A"}}>
          Actual: <span style={{color:"#FFD60A",fontWeight:700}}>{currentMax} cupos</span> · 
